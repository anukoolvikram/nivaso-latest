...
const NoticeForm = ({ notice, onSubmit, onCancel, isSubmitting, noticeTypes, userRole }) => {
  const isEditing = Boolean(notice?.id);
  const [formData, setFormData] = useState({ title: '', content: '', type: '', images: [] });
  const [selectedImages, setSelectedImages] = useState([]);
  const [confirmUpdate, setConfirmUpdate] = useState(false);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const submitData = { ...formData, newImages: selectedImages };
    onSubmit(submitData);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full pb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h2 className="text-2xl font-medium text-navy-dark">{isEditing ? 'Update Notice' : 'New Notice'}</h2>
          <p className="text-sm text-gray-500 font-medium">Keep your neighbors informed.</p>
        </div>
        <div className="flex gap-3 w-full md:w-auto">
          <button type="button" onClick={onCancel} className="flex-1 md:flex-none px-5 py-3 border border-gray-200 bg-white rounded-xl font-bold text-gray-600 flex items-center justify-center gap-2 hover:bg-gray-50 transition">
            <CrossIcon2 /> Cancel
          </button>
          <button 
            type="submit" 
            disabled={isSubmitting || (isEditing && !confirmUpdate) || !isEditing} 
          ...
          >
            {isSubmitting ? <CircularProgress size={20} color="inherit" /> : <><PublishIcon /> {isEditing ? 'Update' : 'Post'}</>}
          </button>
        </div>
      </div>
      ...
      <div className="bg-white p-5 md:p-8 rounded-2xl border border-gray-100 shadow-sm space-y-6">
        <div className="space-y-2">
          ...
          <input 
            ...
            placeholder="e.g., Lost Keys in Garden" 
        ...
        <ImageUploader 
            ...
            onRemoveExisting={(i) => setFormData({...formData, images: formData.images.filter((_, idx) => idx !== i)})} 
        />

        {isEditing && (
          <label className="flex items-center gap-3 p-4 bg-amber-50 rounded-xl cursor-pointer">
            <input type="checkbox" checked={confirmUpdate} onChange={() => setConfirmUpdate(!confirmUpdate)} className="w-5 h-5 accent-navy" />
            <span className="text-sm font-medium text-amber-900 text-left">I am sure I want to modify this notice.</span>
          </label>
        )}
      </div>
    </form>
