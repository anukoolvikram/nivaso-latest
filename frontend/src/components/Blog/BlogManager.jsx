...
const BlogManager = ({ variant = 'society' }) => {
  ...
  const fetchBlogsForTab = useCallback(
    async (tab, currentUser) => {
      if (!currentUser && (tab === 'personal' || tab === 'user')) return;
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen font-montserrat relative">
      {/* Main Content Area */}
      <div
        className={`
          ${isDetailVisible ? 'hidden md:block md:w-2/3' : 'w-full'}
          px-4 md:px-6 py-4 bg-gray-100/50 transition-all duration-200
        `}
      >
        {/* ✅ IMPORTANT FIX:
            Add our own consistent spacing wrapper instead of relying on "gap-6"
            so cards never stick to the top tabs when sidebar opens */}
        <div className="flex flex-col">
          {isFormVisible ? (
            ...
            <div className="flex flex-col gap-8">
              {/* gap-8 makes the space between tab row and cards feel consistent */}
              <BlogList
                blogs={blogs}
      ...
      {/* Sidebar Detail View - On mobile it becomes a full-screen overlay */}
      {isDetailVisible && (
        <div className="fixed inset-0 z-50 bg-white md:relative md:inset-auto md:w-1/3 md:block border-l border-gray-200 overflow-y-auto">
          <BlogDescription
            blog={selectedBlog}
            user={user}
